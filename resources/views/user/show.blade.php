@php
    ...
    $consultationSlot = App\Models\ConsultationSlots::where('faculty_id', $user->id)
        ->where('status', 1)
        ->get();
@endphp
...
<x-layout :header=true :footer=true>
    ...
    @if (Auth::check() and $user->role == 'faculty' and Auth::user()->id != $user->id and Auth::user()->role == 'student')
        {{-- consultation --}}
        {{-- if the user has any slot available in consultationSlot model --}}
        @if ($consultationSlot->count() > 0)
                    ...
                    <div class="px-2 mx-auto sm:px-4 lg:px-6 max-w-7xl">
                        ...
                        <div class="max-w-4xl mx-auto mt-8 md:mt-12">
                            <div class="overflow-hidden bg-base-100/40 shadow rounded-xl">
                                <form action="{{ route('NewConsultation') }}" method="POST">
                                    @csrf
                                    <div class="grid grid-cols-1 md:grid-cols-2">
                                        <div class="px-5 py-6 bg-base-200/60 md:px-8">
                                            ...
                                            <div
                                                ...
                                                <div class="flex-grow">
                                                    <textarea name="message" id="message" cols="30" rows="10" placeholder="Enter the Message" required
                                            ...
                                            <div class="flow-root">
                                                <div class="-my-6 divide-y divide-base-200">
                                                    ...
                                                    <div class="pt-2 pb-4">
                                                        ...
                                                        @php
                                                            // get the available day from consultation slot model which is active, there would be multiple Sunday, Monday, Tuesday, etc.
                                                            $availableDays = App\Models\ConsultationSlots::where('status', 1)
                                                                ->where('faculty_id', $user->id)
                                                                ->get()
                                                                ->pluck('day_of_week')
                                                                ->unique();
                                                            // assign a variable to convert it to number, for sunday it is 0, monday is 1, tuesday is 2, etc.
                                                            $dayNumber = [
                                                                'Sunday' => 0,
                                                                'Monday' => 1,
                                                                'Tuesday' => 2,
                                                            ...
                                                            $availableDays = $availableDays->map(function ($day) use ($dayNumber) {
                                                                return $dayNumber[$day];
                                                            });
                                                            // days to disable - other than available days
                                                            $daysToDisable = collect(range(0, 6))->diff($availableDays);

                                                            // get the slot times available for the faculty ID
                                                            $availableSlots = App\Models\ConsultationSlots::where('status', 1)
                                                                ->where('faculty_id', $faculty->id)
                                                                ->get();

                                                            foreach ($availableSlots as $slot) {
                                                                // get the booked slots in App\Models\Consultations where $slot->id must match slot_id
                                                                $bookedSlots[] = App\Models\Consultations::where('slot_id', $slot->id)
                                                                                    ...
                                                                                    return (
                                                                                        @php
                                                                                            foreach ($daysToDisable as $day) {
                                                                                                echo "date.getDay() === $day ||";
                                                                                            }
                                                                                        @endphp disabledDates.includes(date.getDate())
                                                                                    )
                                                                                },
                                                                            ],
                                                                        })
                                                            ...
                                                            <div>
                                                                ...
                                                                <div class="mt-1">
                                                                    ...
                                                                    <script>
                                                                        ...
                                                                        const faculty_id = '{{ $user->id }}';

                                                                        dateField.addEventListener('change', async (event) => {
                                                                            const date = event.target.value;
                                                                            const response = await fetch(`/api/slot_availability/${faculty_id}/${date}`);
